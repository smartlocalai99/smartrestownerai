...
export async function createSection(title, displayOrder, badgeText, client = getSupabase()) {
  const { data, error } = await client
    .from("menu_sections")
    .insert({ title, display_order: displayOrder, badge_text: badgeText || null })
    .select()
    .single();
...
export async function updateSection(id, patch, client = getSupabase()) {
  ...
  if (patch.badgeText !== undefined) row.badge_text = patch.badgeText || null;
...
export async function createItem(sectionId, fields, displayOrder, client = getSupabase()) {
  const { data, error } = await client
    ...
    .insert({
      ...
      badge_text: fields.badgeText || null,
    })
    .select()
    .single();
  if (error) throw error;
...
export async function updateItem(id, fields, client = getSupabase()) {
  ...
  if (fields.badgeText !== undefined) row.badge_text = fields.badgeText || null;
